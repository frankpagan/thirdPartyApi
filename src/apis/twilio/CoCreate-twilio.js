...
class CoCreateDataTwilio {
	...
	async sendTwilio(socket, data) {
		...
		try{
			this.url_twilio = data_original.data.url ? data_original.data.url : 'https://server.cocreate.app:8088/twilio/actions_twiml';
			...
			switch (type) {
				...
				case 'callRecordingCreate':
					this.callRecordingCreate(socket, type, params);
        			break;
	...
	async callRecordingCreate(socket, type, params) {
		try {
			const { CallSid } = params
			const recording = await twilio.calls(CallSid)
			      .recordings
			      .create()
			      .fetch();
			const response = {
