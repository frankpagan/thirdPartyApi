...
class CoCreateDataStripe {
	...
	init() {
		console.log('00000')
		if (this.wsManager) {
			this.wsManager.on(this.module_id, (socket, data) => this.sendDataStripe(socket, data));
		}
	}

	async sendDataStripe(socket, data) {
		let type = data['type'];
	...
	async stripeBalanceTranscation(socket, type, stripe) {
		...
		utils.send_response(this.wsManager, socket, { "type": type, "response": balanceTransaction }, this.module_id)
	}
...
module.exports = CoCreateDataStripe;
